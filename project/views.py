...
def send_request_view(request):
    ...
    if search_string is not None:
        ...
        html_string = render_to_string('make_choice.html', context=context)
...
def processView(request):
    ...
    inputs,outputs, tt, annotation = getProcessDetails(process, processes_arr, inputs_arr, outputs_arr, tt_arr)
    context = {
        'process' : process, 
       'inputs' : inputs,
       'outputs' : outputs,
    ...
    html_string = render_to_string('process_view.html', context = context)
    return HttpResponse(html_string)
...
# # # 3)si on a encore du temps on integre la cosine similarity
...
suntences = list(['yessine is a web developer', 'wassim is a data scientist'])
vectorizer = CountVectorizer().fit_transform(suntences)
vectors = vectorizer.toarray()
